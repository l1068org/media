...
/* package */ final class RtspMediaPeriod implements MediaPeriod {
  ...
  /* package */ boolean isReady(int trackGroupIndex) {
    ...
    return !suppressRead()
        && queues.get(trackGroupIndex).isReady(wrappers.get(trackGroupIndex).canceled);
  }
  ...
  /* package */ int readData(
    ...
    return queues
        ...
        .read(formatHolder, buffer, readFlags, wrappers.get(sampleQueueIndex).canceled);
  }
  ...
  private static ImmutableList<TrackGroup> buildTrackGroups(
    ...
    for (int i = 0; i < rtspLoaderWrappers.size(); i++) {
      ...
      listBuilder.add(
          new TrackGroup(
              Integer.toString(groupId++), checkNotNull(w.sampleQueue.getUpstreamFormat())));
      for (int j = 0; j < w.extraSampleQueues.size(); j++) {
        listBuilder.add(
            new TrackGroup(
                Integer.toString(groupId++),
                checkNotNull(w.extraSampleQueues.get(j).getUpstreamFormat())));
      }
    }
