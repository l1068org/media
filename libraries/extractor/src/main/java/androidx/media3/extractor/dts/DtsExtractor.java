...
public final class DtsExtractor implements Extractor {
  ...
  public @ReadResult int read(@NonNull ExtractorInput input, @NonNull PositionHolder seekPosition)
      throws IOException {
    if (!initialized) {
  ...
  private int peekFrameTypeAt(ExtractorInput input, byte[] header, int peekOffset)
      throws IOException {
    input.resetPeekPosition();
  ...
  private DtsUtil.DtsHeader readPeekedExtssHeader(ExtractorInput input) throws IOException {
    ...
    if (DtsUtil.getFrameType(DtsUtil.readSyncWord(prefix))
  ...
  @RequiresNonNull({"extractorOutput", "trackOutput"})
  private void setupFromCoreHeader(
      byte[] header,
      int coreFrameSize,
      @Nullable DtsUtil.DtsHeader extssInfo,
    ...
    if (frameDurationUs > 0) {
      ...
      extractorOutput.seekMap(
          new ConstantBitrateSeekMap(
              input.getLength(), input.getPosition(), (int) bitrate, frameSize));
    } else {
  ...
  private int peekNextExtssSize(ExtractorInput input) throws IOException {
    ...
    if (DtsUtil.getFrameType(DtsUtil.readSyncWord(header))
        != DtsUtil.FRAME_TYPE_EXTENSION_SUBSTREAM) {
      return 0;
    }
  ...
  @RequiresNonNull("trackOutput")
  private @ReadResult int outputCoreSample(ExtractorInput input, int coreFrameSize)
      throws IOException {
