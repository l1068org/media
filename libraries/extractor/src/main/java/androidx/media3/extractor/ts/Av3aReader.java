...
public class Av3aReader implements ElementaryStreamReader {
  ...
  private void readHeaderRemainder(ParsableByteArray source) {
    ...
    frameSize = (int) Math.ceil(
        header.totalBitrate / (float) header.samplingRate * header.frameLength / 8);
...
class Av3aAatfFrame {
  ...
  private boolean postHeader() {
    ...
    if (codingProfile == 0) {
      ...
      if (soundBedType == 0) {
        ...
        totalBitrate = objectChannelNumber * ChannelNumConfig.CHANNEL_CONFIG_MONO.bitrateTable.get(
            bitrateIndexPerChannel);
      } else if (soundBedType == 1) {
        channelCount =
            ChannelNumConfig.fromIdx(channelNumberIndex).channelCount + objectChannelNumber;
        totalBitrate = ChannelNumConfig.fromIdx(channelNumberIndex).bitrateTable.get(bitrateIndex) *
            objectChannelNumber *
  ...
  private enum ChannelNumConfig {
    CHANNEL_CONFIG_MONO(0, 1,
        List.of(16000, 32000, 44000, 56000, 64000, 72000, 80000, 96000, 128000, 144000, 164000,
            192000)),
    CHANNEL_CONFIG_STEREO(1, 2,
        List.of(24000, 32000, 48000, 64000, 80000, 96000, 128000, 144000, 192000, 256000, 320000)),
    CHANNEL_CONFIG_MC_5_1(2, 6,
        List.of(192000, 256000, 320000, 384000, 448000, 512000, 640000, 720000, 144000, 96000,
            128000, 160000)),
    CHANNEL_CONFIG_MC_7_1(3, 8,
        List.of(192000, 480000, 256000, 384000, 576000, 640000, 128000, 160000)),
    ...
    CHANNEL_CONFIG_HOA_ORDER2(12, 0,
        List.of(192000, 256000, 320000, 384000, 480000, 512000, 640000)),
